...
public class Processor extends AbstractProcessor {
    ...
    private void generateMetaClass(TypeElement element, RoundEnvironment roundEnv) {
        ...
        debug("Generate meta class by guardman-generator: bean=%s,  meta=%s", classNameOf(element), fqn);
        try {
            ...
            try (SourceWriter w = new SourceWriter(new BufferedWriter(file.openWriter()))) {
                ...
                w.println("import com.github.monzou.guardman.model.BeanProperty;");
                w.println("import %s;", classNameOf(element));
                w.println();
                w.println("/**");
                w.println(" * %s", className);
                w.println(" */");
                w.println("@SuppressWarnings(\"serial\")");
                w.println("public final class %s implements java.io.Serializable {", className);
                w.println();
                w.println("    private static final %s INSTANCE = new %s();", className, className);
                w.println();
                w.println("    public static %s get() {", className);
                ...
                for (VariableElement e : ElementFilter.fieldsIn(element.getEnclosedElements())) {
                    ...
                    w.println("    public final transient BeanProperty<%s, %s> %s = new BeanProperty<%s, %s>() {", beanClassName, propertyType, propertyName, beanClassName, propertyType);
                    w.println();
                ...
                w.println("    private Object readResolve() {");
                w.println("        return INSTANCE;");
                w.println("    }");
                w.println();
                w.println("    private %s() {", className);
                w.println("    }");
                w.println();
                w.println("}");
                w.flush();
    ...
    private String getterNameOf(VariableElement element, TypeMirror mirror) {
        ...
        StringBuilder capitalizedName = new StringBuilder().append(Character.toTitleCase(propertyName.charAt(0))).append(propertyName.substring(1));
